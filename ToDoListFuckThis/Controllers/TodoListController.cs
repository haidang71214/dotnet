    ...
    public class TodoListController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse>> UpdateTodo(string id, [FromBody] TodolistUpdateRequestDto dto)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dto.Name))
                todo.Name = dto.Name.Trim();

            if (!string.IsNullOrWhiteSpace(dto.Comment))
                todo.Comment = dto.Comment.Trim();

            if (dto.Priority.HasValue && dto.Priority != PriorityEnums.CLEAR)
                todo.Priority = dto.Priority.Value;

            if (dto.TaskStatus.HasValue && dto.TaskStatus != TaskStatusEnum.PROGRESS)
                todo.TaskStatus = dto.TaskStatus.Value;

            if (dto.timeStart.HasValue)
                todo.timeStart = dto.timeStart.Value.ToUniversalTime();

            if (dto.timeEnd.HasValue)
                todo.timeEnd = dto.timeEnd.Value.ToUniversalTime();
